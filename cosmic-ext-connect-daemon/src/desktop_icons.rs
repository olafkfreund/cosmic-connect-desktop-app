...
use cosmic_ext_connect_protocol::{Device, DeviceType};
use std::fs;
...
Icon={icon}
Exec=cosmic-ext-connect-manager --select-device {device_id} %U
Terminal=false
...
Name=Send File
Exec=cosmic-ext-connect-manager --select-device {device_id} --tab share

[Desktop Action Ping]
Name=Ping Device
Exec=cosmic-ext-connect-manager --select-device {device_id} --device-action ping

[Desktop Action Find]
Name=Find Device
Exec=cosmic-ext-connect-manager --select-device {device_id} --device-action find

[Desktop Action Browse]
Name=Browse Files
Exec=cosmic-ext-connect-manager --select-device {device_id} --tab files
"#,
        name = device_name,
        status = status,
...
pub fn get_desktop_icon_path(device_id: &str) -> PathBuf {
    ...
    applications_dir.join(format!("cosmic-ext-connect-{}.desktop", device_id))
}
...
pub fn get_user_desktop_icon_path(device_id: &str) -> PathBuf {
    ...
    desktop_dir.join(format!("cosmic-ext-connect-{}.desktop", device_id))
}
...
mod tests {
    use super::*;
    use cosmic_ext_connect_protocol::{DeviceInfo, PairingStatus};
    use std::fs;
    ...
    fn create_test_device() -> Device {
        ...
        Device::new(
            info,
            cosmic_ext_connect_protocol::ConnectionState::Connected,
    ...
    fn test_desktop_icon_path() {
        ...
        assert!(path
            .to_string_lossy()
            .contains("cosmic-ext-connect-test_device_123.desktop"));
    }
    ...
    fn test_device_status_in_comment() {
        ...
        paired.connection_state = cosmic_ext_connect_protocol::ConnectionState::Disconnected;
