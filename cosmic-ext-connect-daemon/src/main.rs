...
use cosmic_ext_connect_protocol::plugins::extendeddisplay::ExtendedDisplayPluginFactory;
use cosmic_ext_connect_protocol::plugins::remotedesktop::RemoteDesktopPluginFactory;
...
struct Daemon {
    ...
    tls_config: Arc<cosmic_ext_connect_protocol::TlsConfig>,
...
impl Daemon {
    ...
    async fn new(config: Config) -> Result<Self> {
        ...
        let tls_config = Arc::new(
            cosmic_ext_connect_protocol::TlsConfig::new(&certificate)
    ...
    async fn handle_pairing_event(
        ...
        match event {
            ...
            PairingEvent::PairingTimeout { device_id } => {
                ...
                let error = cosmic_ext_connect_protocol::ProtocolError::Timeout(
            ...
            PairingEvent::Error { device_id, message } => {
                ...
                let error = cosmic_ext_connect_protocol::ProtocolError::NetworkError(message);
                error_handler
                ...
                *receiver_guard = Some(rx);
                ...
                tokio::spawn(async move {
                    ...
                    while let Some(notification) = receiver.recv().await {
                        ...
                        use cosmic_ext_connect_protocol::plugins::notification::{
                                ...
                                .await
                            {
                                ...
                                if let Some(device_config) = config_registry.get(&device_id) {
                                    if let Some(mac_address) = device_config.get_mac_address() {
                                        use cosmic_ext_connect_protocol::plugins::wol::WolPlugin;
            ...
            ConnectionEvent::PacketReceived {
                ...
                if let Some(device) = dev_manager.get_device_mut(&device_id) {
                    ...
                    if packet.packet_type == "cconnect.camera.frame" {
                        ...
                        use cosmic_ext_connect_protocol::plugins::camera::CameraPlugin;
    ...
    fn convert_player_state(
        ...
        cosmic_ext_connect_protocol::plugins::mpris::PlayerStatus,
        cosmic_ext_connect_protocol::plugins::mpris::PlayerMetadata,
    ) {
        use cosmic_ext_connect_protocol::plugins::mpris::{
    ...
    async fn handle_mpris_request(
        ...
        let send_mpris_packet = |packet: cosmic_ext_connect_protocol::Packet| async move {
                ...
                if should_connect {
                    ...
                    if now.duration_since(*last_attempt) >= backoff {
                        ...
                        *count += 1;
                        ...
                        if let cosmic_ext_connect_protocol::transport::TransportAddress::Tcp(addr) =
                            transport_address
                        {
                            let device_id_clone = device_id.clone();
