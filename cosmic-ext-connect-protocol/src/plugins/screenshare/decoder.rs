...
impl VideoDecoder {
    ...
    pub fn start(&self) -> Result<()> {
        self.pipeline.set_state(gst::State::Playing).map_err(|e| {
            crate::ProtocolError::Plugin(format!("Failed to start pipeline: {}", e))
        })?;
    ...
    pub fn push_frame(&self, data: &[u8]) -> Result<()> {
        ...
        if let Err(e) = self.appsrc.push_buffer(buffer) {
            ...
            return Err(crate::ProtocolError::Plugin(format!(
    ...
    pub fn pull_frame(&self) -> Result<Option<(Vec<u8>, u32, u32)>> {
        ...
        match self
            .appsink
            .try_pull_sample(gst::ClockTime::from_mseconds(5))
        {
            Some(sample) => {
                let caps = sample
                    .caps()
                    .ok_or_else(|| crate::ProtocolError::Plugin("No caps in sample".to_string()))?;
                let structure = caps.structure(0).ok_or_else(|| {
                    crate::ProtocolError::Plugin("No structure in caps".to_string())
                })?;

                let width = structure
                    .get::<i32>("width")
                    .map_err(|_| crate::ProtocolError::Plugin("No width in caps".to_string()))?
                    as u32;
                let height = structure
                    .get::<i32>("height")
                    .map_err(|_| crate::ProtocolError::Plugin("No height in caps".to_string()))?
                    as u32;

                let buffer = sample.buffer().ok_or_else(|| {
                    crate::ProtocolError::Plugin("No buffer in sample".to_string())
                })?;
                let map = buffer.map_readable().map_err(|_| {
                    crate::ProtocolError::Plugin("Failed to map buffer".to_string())
                })?;
