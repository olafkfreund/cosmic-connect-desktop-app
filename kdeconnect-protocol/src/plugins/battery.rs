...
//!
//! ```rust,ignore
//! use kdeconnect_protocol::plugins::battery::{BatteryPlugin, BatteryStatus};
...
///
/// ```rust
/// use kdeconnect_protocol::plugins::battery::BatteryStatus;
///
/// let status = BatteryStatus {
///     current_charge: 75,
///     is_charging: true,
...
impl BatteryPlugin {
    ...
    ///
    /// ```rust
    /// use kdeconnect_protocol::plugins::battery::BatteryPlugin;
    ///
    /// let plugin = BatteryPlugin::new();
    /// assert!(plugin.get_battery_status().is_none());
    /// ```
