...
#[derive(Parser, Debug)]
#[command(name = "cosmic-ext-messages-popup")]
...
mod tests {
    ...
    fn test_args_parsing() {
        let args = Args::parse_from(["cosmic-ext-messages-popup"]);
        assert!(!args.show);
    ...
    fn test_args_with_options() {
        let args = Args::parse_from(["cosmic-ext-messages-popup", "--show", "--messenger", "whatsapp"]);
        assert!(args.show);
