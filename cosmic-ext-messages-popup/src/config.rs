...
impl Config {
    ...
    pub fn config_path() -> Option<PathBuf> {
        dirs::config_dir().map(|dir| dir.join("cosmic").join("io.github.olafkfreund.CosmicExtMessagesPopup.ron"))
    }

    /// Get the data directory for WebView storage
    pub fn data_dir() -> Option<PathBuf> {
        dirs::data_local_dir().map(|dir| dir.join("cosmic-ext-messages-popup"))
    }
