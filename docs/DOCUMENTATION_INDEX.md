# COSMIC Connect Documentation Index

Complete listing of all documentation files organized by category.

##  Main Navigation

**Start Here**: [Home.md](Home.md) - Main documentation hub with full navigation

##  Complete File Index

###  Architecture & Design (2 files)

| File | Description |
|------|-------------|
| [Architecture.md](architecture/Architecture.md) | Multi-platform architecture, shared core, integration |
| [Protocol.md](architecture/Protocol.md) | KDE Connect protocol v7/8 specification |

###  Development Guides (5 files)

| File | Description |
|------|-------------|
| [Contributing.md](development/Contributing.md) | Contribution guidelines, code style, workflow |
| [Development-Guide.md](development/Development-Guide.md) | Complete development documentation |
| [Setup.md](development/Setup.md) | Environment setup for NixOS and other distros |
| [Build-Fixes.md](development/Build-Fixes.md) | Common build issues and solutions |
| [Applet-Development.md](development/Applet-Development.md) | COSMIC panel applet development guide |

###  Project Management (5 files)

| File | Description |
|------|-------------|
| [Status.md](project/Status.md) | Current implementation status and roadmap |
| [Acceptance-Criteria.md](project/Acceptance-Criteria.md) | Quality standards and definition of done |
| [Issues.md](project/Issues.md) | GitHub issue management and tracking |
| [Issue-37-Breakdown.md](project/Issue-37-Breakdown.md) | Detailed breakdown of major issue #37 |
| [Issues-To-Close.md](project/Issues-To-Close.md) | List of issues ready to close |

### 📚 User Documentation (5 files)

| File | Description |
|------|-------------|
| [USER_GUIDE.md](USER_GUIDE.md) | End-user setup and usage instructions |
| [INSTALL.md](INSTALL.md) | Installation instructions |
| [PAIRING_PROCESS.md](PAIRING_PROCESS.md) | Device pairing guide |
| [TROUBLESHOOTING.md](TROUBLESHOOTING.md) | Common issues and solutions |
| [PLUGIN_TESTING_GUIDE.md](PLUGIN_TESTING_GUIDE.md) | Plugin testing documentation |

###  Technical Documentation (3 files)

| File | Description |
|------|-------------|
| [TLS_IMPLEMENTATION.md](TLS_IMPLEMENTATION.md) | TLS/certificate implementation details |
| [PAIRING_FIX_RETROSPECTIVE.md](PAIRING_FIX_RETROSPECTIVE.md) | Pairing implementation retrospective |
| [issue-52-fix.md](issue-52-fix.md) | Connection stability fix documentation |

###  Repository Documentation (2 files)

| File | Description |
|------|-------------|
| [README.md](README.md) | Documentation directory overview |
| [WIKI_SETUP.md](WIKI_SETUP.md) | GitHub Wiki setup guide (optional) |

###  Archive (1 file)

| File | Description |
|------|-------------|
| [old-context.md](archive/old-context.md) | Historical project context (outdated) |

##  Quick Access by Topic

### For New Contributors
1. [Home.md](Home.md) - Start here
2. [Contributing.md](development/Contributing.md) - How to contribute
3. [Setup.md](development/Setup.md) - Environment setup
4. [Development-Guide.md](development/Development-Guide.md) - Development workflow

### For Understanding Architecture
1. [Architecture.md](architecture/Architecture.md) - System design
2. [Protocol.md](architecture/Protocol.md) - Protocol details
3. [TLS_IMPLEMENTATION.md](TLS_IMPLEMENTATION.md) - TLS implementation

### For End Users
1. [USER_GUIDE.md](USER_GUIDE.md) - User guide
2. [INSTALL.md](INSTALL.md) - Installation
3. [PAIRING_PROCESS.md](PAIRING_PROCESS.md) - Device pairing
4. [TROUBLESHOOTING.md](TROUBLESHOOTING.md) - Troubleshooting

### For Project Management
1. [Status.md](project/Status.md) - Current status
2. [Acceptance-Criteria.md](project/Acceptance-Criteria.md) - Quality standards
3. [Issues.md](project/Issues.md) - Issue tracking

##  Statistics

- **Total Documentation Files**: 25+ markdown files
- **Categories**: 7 main categories
- **Architecture Docs**: 2 files
- **Development Guides**: 5 files
- **Project Management**: 5 files
- **User Documentation**: 5 files
- **Technical Docs**: 3 files

##  Maintenance

This index is manually maintained. When adding new documentation:

1. Create the file in the appropriate category directory
2. Update [Home.md](Home.md) with a link
3. Update this index file
4. Update [README.md](README.md) if needed

##  External Links

- [Main Project README](../README.md)
- [cosmic-ext-connect-core](https://github.com/olafkfreund/cosmic-ext-connect-core)
- [cosmic-connect-android](https://github.com/olafkfreund/cosmic-connect-android)

---

**Last Updated**: 2026-01-15

**Navigation**: [↑ Back to Top](#cosmic-connect-documentation-index) | [← Home](Home.md) | [← Main README](../README.md)
